...
class Ui_MainWindow(QMainWindow):
    ...
    def query(self):
        ...
        result = dboperation.query("select * from tb_grade")
        row = len(result)
    ...
    def editLimit(self):
        ...
        self.lineEdit_2.setPlaceholderText("例如: 本科二年级")
    # 添加年级信息
    def add(self):
        # 1. 判断输入是否为空
        ...
        if gradeID != "" and gradeName != "":
            # 2. 判断是否年级信息已存在
            if not self.ifExistGradeInfo(gradeID, gradeName):
                res = dboperation.exec("insert into tb_grade values(%s, %s)", (gradeID, gradeName))
                if res > 0: # 3. 刷新表格
                    self.query()
            else:
                QMessageBox.information(self, "提示", "您添加的年级已录入, 请勿重复输入")
        
    def ifExistGradeInfo(self, gradeID, gradeName):
        resid = dboperation.query(f"select * from tb_grade where gradeID = '{gradeID}'")
        resna = dboperation.query(f"select * from tb_grade where gradeName = '{gradeName}'")
    ...
    def delitem(self):
        # 依据 id 删除, 注意同时要删除: 年级下对应的班级信息, 以及所设立的学科
        try:
            # self.rid 可能不存在, 也即未选中
            isok = dboperation.exec("delete from tb_grade where gradeID= %s", (self.rid,))
            isok = dboperation.exec("delete from tb_class where gradeID= %s", (self.rid,))
            isok = dboperation.exec("delete from tb_subject where gradeName= %s", (self.rname,))
            if isok > 0:
                self.query()
        except:
    ...
    def upditem(self):
        ...
        try:
            ...
            if uname == self.rname:
                ...
                isok = dboperation.exec("update tb_grade set gradeName= %s where gradeID=%s", (uname, self.rid))
                if isok > 0:
                    self.query()
        except:
            QMessageBox.information(self, "提示", "请先选择表格的一行")
    ...
    def setupUi(self, MainWindow):
        ...
        self.label.setGeometry(QtCore.QRect(120, 360, 54, 20))
        self.label.setObjectName("label")
        self.lineEdit = QtWidgets.QLineEdit(parent=self.centralwidget)
        self.lineEdit.setGeometry(QtCore.QRect(180, 360, 113, 20))
        self.lineEdit.setObjectName("lineEdit")
        self.label_2 = QtWidgets.QLabel(parent=self.centralwidget)
        self.label_2.setGeometry(QtCore.QRect(360, 360, 54, 20))
        self.label_2.setObjectName("label_2")
        self.lineEdit_2 = QtWidgets.QLineEdit(parent=self.centralwidget)
        self.lineEdit_2.setGeometry(QtCore.QRect(420, 360, 113, 20))
